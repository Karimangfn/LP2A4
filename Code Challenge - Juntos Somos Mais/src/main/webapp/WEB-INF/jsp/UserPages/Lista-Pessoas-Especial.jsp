...
<meta charset="UTF-8">
<title>Lista de Pessoas Especial</title>
</head>
<body>
	<div class="Header">
		...
		<div class="container-size">
			<div class="left-box">
				<div class="filters">
					...
					<div class="inputs">
						<input class="radio" type="radio" name="filter" id="Especial" checked></input>
			...
			<div class="right-box">
				...
				<c:forEach var="listpicture" items="${listpicture}" varStatus="status">
					<c:if test="${listcoordinates[status.index].longitude <= '-2.196998'}">
					<c:if test="${listcoordinates[status.index].longitude >= '-15.411580'}">
					<c:if test="${listcoordinates[status.index].latitude >= '-46.361899'}">
					<c:if test="${listcoordinates[status.index].latitude >= '-34.276938'}">
					<div class="card">
