...
<meta charset="UTF-8">
<title>Descricao das Pessoas</title>
</head>
<body>


	<c:forEach var="listResultsIdPicture" items="${listResultsId.picture}"
		varStatus="status">
		<img src="${listResultsIdPicture.large}">
	</c:forEach>

	<c:forEach var="listResultsIdName" items="${listResultsId.name}"
		varStatus="status">
		<h3>Nome: ${listResultsIdName.first} ${listResultsIdName.last}</h3>
	</c:forEach>

	<c:forEach var="listResultsId" items="${listResultsId.gender}"
		varStatus="status">
		<h3>Sexo: ${listResultsId}</h3>
	</c:forEach>

	<c:forEach var="listResultsId" items="${listResultsId.nationality}"
		varStatus="status">
		<h3>Nacionalidade: ${listResultsId}</h3>
	</c:forEach>

	<c:forEach var="listResultsIdDob" items="${listResultsId.dob}"
		varStatus="status">
		<h3>Data de Nascimento: ${listResultsIdDob.date}</h3>
	</c:forEach>

	<c:forEach var="listResultsIdRegistered"
		items="${listResultsId.registered}" varStatus="status">
		<h3>Data de Registro: ${listResultsIdRegistered.date}</h3>
	</c:forEach>

	<c:forEach var="listResultsId" items="${listResultsId.email}"
		varStatus="status">
		<h3>Email: ${listResultsId}</h3>
	</c:forEach>

	<c:forEach var="listResultsId" items="${listResultsId.phone}"
		varStatus="status">
		<h3>Telefone: ${listResultsId}</h3>
	</c:forEach>

	<c:forEach var="listResultsId" items="${listResultsId.cell}"
		varStatus="status">
		<h3>Celular: ${listResultsId}</h3>
	</c:forEach>

	<c:forEach var="listResultsIdLocation"
		items="${listResultsId.location}" varStatus="status">
	...
	<c:forEach var="listResultsIdLocation"
		items="${listResultsId.location}" varStatus="status">
		<c:forEach var="listResultsIdCoordinates"
			items="${listResultsIdLocation.coordinates}" varStatus="status">
			<h3>
				Coordenadas: <br> 
				Latitude: ${listResultsIdCoordinates.latitude} <br> 
				Longitude: ${listResultsIdCoordinates.longitude}
			</h3>
		</c:forEach>
	</c:forEach>
</body>
